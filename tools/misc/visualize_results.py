...
class ResultVisualizer:
    ...
    def evaluate_and_show(self,
        ...
        if isinstance(results[0], list):
            samples = self.detection_evaluate(dataset, results)
        elif isinstance(results[0], tuple):
            results_ = [result[0] for result in results]
            samples = self.detection_evaluate(dataset, results_)
        else:
        ...
        self._save_image_gts_results(dataset, results, samples, out_dir)

    def detection_evaluate(self, dataset, results, eval_fn=None):
...
def main():
    ...
    result_visualizer.evaluate_and_show(dataset, outputs, show_dir=args.show_dir)
