...
class LocalTransformerBlock(nn.Module):
    def __init__(self, dim, window_size, shift_size, bias):
        ...
        # self.project_out1 = nn.Conv2d(dim, dim, kernel_size=1, padding=0, bias=bias)
        
        # self.norm1 = nn.LayerNorm(dim)
        self.qkv1 = nn.Linear(dim, dim * 3, bias=bias)
    ...
    def forward(self, x):
        ...
        q = rearrange(q, 'b (h w) c -> b c (h w)', h=self.window_size, w=self.window_size)
        k = rearrange(k, 'b (h w) c -> b c (h w)', h=self.window_size, w=self.window_size)
        v = rearrange(v, 'b (h w) c -> b c (h w)', h=self.window_size, w=self.window_size)
        ...
        # qkv = self.qkv_conv1(shift_window)
        # q, k, v = qkv.chunk(3, dim=1)
        #
        # q = rearrange(q, 'b c h w -> b c (h w)')
        # k = rearrange(k, 'b c h w -> b c (h w)')
        ...
        shift_window = rearrange(shift_window, 'b c h w -> b (h w) c')
        # shift_window = self.norm1(shift_window)
        qkv = self.qkv1(shift_window)
        q, k, v = qkv.chunk(3, dim=-1)
...
class WaterFormerV5(BaseModule):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=36,
