...
class WaterFormerV3NoSkip(BaseModule):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=36,
        ...
        self.skip_connect3 = nn.Identity()#nn.Conv2d(int(dim * 2 ** 2), int(dim * 2 ** 2), kernel_size=1, bias=bias)
        # self.decoder_level3 = nn.Sequential(*[
        #     TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], bias=bias, window_size=window_size,
        #                      shift_size=shift_size) for i in range(num_blocks[2])])
        self.decoder_level3 = nn.Sequential(*[
        ...
        self.skip_connect2 = nn.Identity() #nn.Conv2d(int(dim * 2 ** 1), int(dim * 2 ** 1), kernel_size=1, bias=bias)
        # self.decoder_level2 = nn.Sequential(*[
        #     TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], bias=bias, window_size=window_size,
        #                      shift_size=shift_size) for i in range(num_blocks[1])])
        self.decoder_level2 = nn.Sequential(*[
        ...
        self.skip_connect1 = nn.Identity() #nn.Conv2d(int(dim), int(dim), kernel_size=1, bias=bias)
        
        # self.decoder_level1 = nn.Sequential(*[
        #     TransformerBlock(dim=int(dim), num_heads=heads[0], bias=bias, window_size=window_size,
        ...
        # self.reduce_chan_ref = nn.Conv2d(int(dim*2), int(dim//2), kernel_size=1, bias=bias)
        self.refinement = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2), num_heads=heads[0], bias=bias, window_size=window_size,
                             shift_size=shift_size) for i in range(num_refinement_blocks)])
