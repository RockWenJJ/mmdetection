...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size, shift_size, bias):
        ...
        self.glob_attn = ChannelAttention(dim, num_heads, bias)
        # self.glob_norm2 = LayerNorm(dim)
        # self.glob_ffn = FeedForward(dim, bias)
        
        # local transformer
        # self.pixel = SWPSATransformerBlock(dim,window_size,shift_size,bias)
        self.local_norm1 = LayerNorm(dim)
        self.local_attn = SWPSA(dim, window_size, shift_size, bias)
        # self.local_norm2 = LayerNorm(dim)
        # self.local_ffn = FeedForward(dim, bias)
    ...
    def forward(self, x):
        ...
        # y1 = shortcut + self.glob_ffn(self.glob_norm2(y1))
        y1 = shortcut + y1
        
        
        y2 = self.local_attn(self.local_norm1(x))
        # y2 = shortcut + self.local_ffn(self.local_norm2(y2))
        y2 = shortcut + y2
...
class WaterFormerV2(BaseModule):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=36,
        ...
        self.encoder_level2 = nn.Sequential(*[
            TransformerBlock(dim=dim * 2**1, num_heads=heads[1], bias=bias,window_size=window_size,shift_size=shift_size) for i in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))  ## From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(*[
            TransformerBlock(dim=dim * 2**2, num_heads=heads[2], bias=bias,window_size=window_size,shift_size=shift_size) for i in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim * 2 ** 2))  ## From Level 3 to Level 4
        self.latent = nn.Sequential(*[
            TransformerBlock(dim=dim * 2**3, num_heads=heads[3], bias=bias,window_size=window_size,shift_size=shift_size) for i in range(num_blocks[3])])

        self.up4_3 = Upsample(int(dim * 2 ** 3))  ## From Level 4 to Level 3
        self.reduce_chan_level3 = nn.Conv2d(int(dim * 2 ** 3), int(dim * 2 ** 2), kernel_size=1, bias=bias)
        self.decoder_level3 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], bias=bias, window_size=window_size,shift_size=shift_size) for i in range(num_blocks[2])])

        self.up3_2 = Upsample(int(dim * 2 ** 2))  ## From Level 3 to Level 2
        self.reduce_chan_level2 = nn.Conv2d(int(dim * 2 ** 2), int(dim * 2 ** 1), kernel_size=1, bias=bias)
        self.decoder_level2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], bias=bias, window_size=window_size,shift_size=shift_size) for i in range(num_blocks[1])])
        ...
        self.refinement = nn.Sequential(*[
            TransformerBlock(dim=int(dim), num_heads=heads[0], bias=bias, window_size=window_size,shift_size=shift_size) for i in range(num_refinement_blocks)])
