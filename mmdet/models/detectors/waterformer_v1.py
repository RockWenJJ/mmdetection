...
class ConvProjection(nn.Module):
    ...
    def forward(self, x, hw_shape, attn_kv=None):
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        B_, N, C = x.shape
        q, k, v = self.qkv(x, hw_shape=self.win_size, attn_kv=attn_kv)
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 ...
                 modulator=False, cross_modulator=False, bottleneck=False):
        super().__init__()
        self.dim = dim
        ...
        self.c_attn = ChannelWiseSelfAttention(dim, num_heads=num_heads, bias=True)
        # self.cw_attn = WindowAttention(
        #     dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
        #     qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
        #     token_projection=token_projection)
