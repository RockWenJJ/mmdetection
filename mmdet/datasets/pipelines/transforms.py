...
class RandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        """
        assert crop_size[0] > 0 and crop_size[1] > 0
        # make sure the same crop region for all img_fields
        img = results['img']
        ...
        ## wenjj: record cropped positions
        results['crop_xyxy'] = np.array([crop_x1, crop_y1, crop_x2, crop_y2])
...
## wenjj: crop in the image center
@PIPELINES.register_module()
class CenterCrop:
    """Center crop the image & bboxes & masks.
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            # offset_h = np.random.randint(0, margin_h + 1)
            # offset_w = np.random.randint(0, margin_w + 1)
            offset_h = max(margin_h // 2, 0)
            offset_w = max(margin_w // 2, 0)
        ...
        # ## wenjj: generate fake scale factor
