...
class LoadSynthesisFromFile:
    ...
    def __call__(self, results):
        ...
        if results['syn_prefix'] is not None:
            filename = osp.join(results['syn_prefix'],
                                results['img_info']['filename'])
        else:
            filename = results['img_info']['filename']
        ...
        results['syn_img0'] = img
        results['syn_num'] = 1
        results['img_fields'].append('syn_img0')
        assert results['syn_img0'].shape == results['img'].shape, 'Synthesis image should have the same shape with ori image'
        return results
...
@PIPELINES.register_module()
class LoadBackFromFile:
    """Load a backscattering from file.
    ...
    def __call__(self, results):
        ...
        if results['back_prefix'] is not None:
            filename = osp.join(results['back_prefix'],
                                results['img_info']['filename'])
        else:
            filename = results['img_info']['filename']
        ...
        results['back0'] = img
        results['img_fields'].append('back0')
        assert results['back0'].shape == results['img'].shape, 'Backscattering image should have the same shape with ori image'
        return results
...
class LoadDepthFromFile:
    ...
    def __call__(self, results):
        ...
        if results['dep_prefix'] is not None:
        ...
        results['depth'] = img
        results['img_fields'].append('depth')
        # assert results['depth'].shape == results['img'].shape, 'Backscattering image should have the same shape with ori image'
        return results
